...
import api_omnilink
import api_raster
from supabase_db import select_rows, test_connection
...
.hero-title {font-size: 40px; font-weight: 900; letter-spacing: -0.05em; margin: 0; color: var(--text);}
.hero-subtitle {font-size: 15px; color: #CBD5E1; margin: 4px 0 0 0;}
...
.kpi-value {font-size: 31px; font-weight: 900; color: var(--text); margin-top: 8px; line-height: 1.05;}
.kpi-help {font-size: 12px; color: #64748B; margin-top: 10px;}
.badge {
    display: inline-flex;
    align-items: center;
    gap: 6px;
...
.badge.ok { border-color: rgba(34,197,94,.35); color: #BBF7D0; background: rgba(34,197,94,.10); }
...
.metric-tile-label {font-size: 12px; color: var(--muted);}
.metric-tile-value {font-size: 22px; font-weight: 800; color: var(--text); margin-top: 6px;}
...
@media (max-width: 1200px) {
    .kpi-row-7 { grid-template-columns: repeat(3, minmax(0,1fr)); }
}
...
def require_login() -> bool:
    user_ok = os.getenv("DASHBOARD_USER", "Admin").strip().strip('"')
    pass_ok = os.getenv("DASHBOARD_PASSWORD", "inicio@01A").strip().strip('"')
